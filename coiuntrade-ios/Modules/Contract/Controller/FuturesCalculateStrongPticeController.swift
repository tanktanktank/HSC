...
class FuturesCalculateStrongPticeController: BaseViewController {
    ...
    let openPriceView = FuturesTitleInputView(title: "开仓价格")// 开仓价格
    let closePriceView = FuturesTitleInputView(title: "平仓价格")// 平仓价格
    let amountView = FuturesTitleInputView(title: "平仓价格")// 平仓价格

    let estPriceLabel : TitleValueLabelView = {
        let maxHoldLabel = TitleValueLabelView()
        maxHoldLabel.title = "强平参考价"
        maxHoldLabel.value = "250,000 USDT"
        return maxHoldLabel
    }()
...
extension FuturesCalculateStrongPticeController {
    ...
    func setUI(){
        ...
        typeBtn.dataSource = ["全仓","逐仓"]
        holdDirectionBtn.textAligment = .center
        holdDirectionBtn.dataSource = ["单向持仓","双向持仓"]
