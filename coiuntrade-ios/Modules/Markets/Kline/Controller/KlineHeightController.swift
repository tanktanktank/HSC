...
class KlineHeightController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        slider.rx.value.asObservable()
            .subscribe(onNext: { [self] value in
                //0.5 ~1~ 1.5;   0.0 ~0.5~ 1.0
                
                var updateValue = CGFloat(value)
                if value < 0.3 {
                    self.heightLayout.constant = SCREEN_WIDTH / (375.0 / (328.0 * 0.3))
                    updateValue = 0.3
                } else{
                    self.heightLayout.constant = SCREEN_WIDTH / (375.0 / (328.0 * CGFloat(value)))
                }
                let defaults = UserDefaults.standard
