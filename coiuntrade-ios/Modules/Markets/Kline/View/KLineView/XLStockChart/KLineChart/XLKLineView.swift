...
@objc public protocol XLKLineViewProtocol: class {
    ...
    @objc optional func XLKLineViewLongPress(model: XLKLineModel, preClose: CGFloat, point: CGPoint)
}
...
open class XLKLineView: UIView, XLDrawLayerProtocol {
    ...
    /// 轻触打开
    var isLighterShow: Bool = true
    /// 滑动震动
    var isShockShow: Bool = true
...
//        print("currentWidth " + "\(kLineViewWidth)")
        ...
        kLineViewWidth += (SCREEN_WIDTH - 90)
    ...
    /// 加载k线数据的方法 mainDrawString：主图绘制字符串，secondDrawString：副图绘制字符串，dateType：日期显示类型，lineType：K线显示类型
    public func configureViewNoOffset(data: [XLKLineModel], isNew: Bool, mainDrawString: String, secondDrawString: String, dateType: XLKLineDateType, lineType: XLKLineType) {
...
//        print("currentWidth " + "\(kLineViewWidth)")
    ...
    func updateKlineColor(newColors: [UIColor]){
        kLine.theme.topTextOneColor = newColors[0]
        kLine.theme.topTextTwoColor = newColors[1]
        kLine.theme.topTextThreeColor = newColors[2]
    }
    func updateKlineWidth(newLineWidths: [Float]){
        kLine.theme.topLineOneWidth = Double((newLineWidths[0] + 1.0))
        kLine.theme.topLineTwoWidth = Double((newLineWidths[1] + 1.0))
        kLine.theme.topLineThreeWidth = Double((newLineWidths[2] + 1.0))
    }
    func updateBollKLineColor(newColors: [UIColor]){
        kLine.theme.topBollUpColor = newColors[0]
        kLine.theme.topBollMidColor = newColors[1]
        kLine.theme.topBollBottomColor = newColors[2]
    }
    func updateBollKLineWidth(newLineWidths: [Float]){
        kLine.theme.topBollUpWidth = Double((newLineWidths[0] + 1.0))
        kLine.theme.topBollMidWidth = Double((newLineWidths[1] + 2.0))
        kLine.theme.topBollBottomWidth = Double((newLineWidths[2] + 3.0))
    }
    func updateMACDKLineColor(newColors: [UIColor]){
        kLine.theme.bottomMACDDifColor = newColors[0]
        kLine.theme.bottomMACDDeaColor = newColors[1]
    }
    func updateMACDKLineWidth(newLineWidths: [Float]){
        kLine.theme.bottomMACDDifLW = Double((newLineWidths[0] + 1.0))
        kLine.theme.bottomMACDDeaLW = Double((newLineWidths[1] + 1.0))
    }
    func updateKDJKLineColor(newColors: [UIColor]){
        kLine.theme.bottomKDJKColor = newColors[0]
        kLine.theme.bottomKDJDColor = newColors[1]
        kLine.theme.bottomKDJJColor = newColors[2]
    }
    func updateKDJKLineWidth(newLineWidths: [Float]){
        kLine.theme.bottomKDJKLineW = Double((newLineWidths[0] + 1.0))
        kLine.theme.bottomKDJDLineW = Double((newLineWidths[1] + 1.0))
        kLine.theme.bottomKDJJLineW = Double((newLineWidths[2] + 1.0))
    }
    func updateRSIKLineColor(newColors: [UIColor]){
        kLine.theme.bottomRSIRColor = newColors[0]
    }
    func updateRSIKLineWidth(newLineWidths: [Float]){
        kLine.theme.bottomRSIRLineW = Double((newLineWidths[0] + 1.0))
    }
        ...
        if highLightIndex < kLine.dataK.count && positionModelIndex < kLine.positionModels.count {
            ...
            if touchY >= topChartTop && touchY <= midTextTop {
                ...
                if let kLineViewDelegate = self.kLineViewDelegate {
                    ...
                    kLineViewDelegate.XLKLineViewLongPress?(model: entity, preClose: preData.close, point: recognizer.location(in: window))
                }
