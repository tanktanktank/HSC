...
extension CHDepthChartView {
    ...
    fileprivate func drawXAxis(){
        ...
        let xLineCount = 4
        var xPoxW:CGFloat = (self.width-self.padding.left-self.padding.right)/CGFloat(xLineCount)
        for xc in 1...(xLineCount-1){
            ...
            //处理辅助线样式
            switch self.xAxis.referenceStyle {
                case let .dash(color: dashColor, pattern: pattern):
                referenceLayer.strokeColor = dashColor.cgColor
                referenceLayer.lineDashPattern = pattern
                showXAxisReference = true
            ...
            //需要画x轴上的辅助线
            if showXAxisReference {
                referencePath.move(to: CGPoint(x: xPoxW * CGFloat(xc)  , y: self.frame.minY-10))
                referencePath.addLine(to: CGPoint(x: xPoxW * CGFloat(xc) , y: self.frame.maxY-10))
                referenceLayer.path = referencePath.cgPath
                self.drawLayer.addSublayer(referenceLayer)
            }
        }
    ...
    func drawChart() -> [(CGRect, String)]{
        ...
        guard self.askItems.count > 0 else {
            return []
        }
