...
open class MarketDepthData: NSObject {
    ...
    convenience init(datas: Array<String>, currencyType: String, exchangeType: ExchangeType) {
        self.init()
        self.currencyType = currencyType
        self.exchangeType = exchangeType
        // 修正小数位精度丢失
        let multiplierPrice = NSDecimalNumber.init(string: datas[0])
        self.price = multiplierPrice.doubleValue
        
        // 修正小数位精度丢失
        let multiplierNumber = NSDecimalNumber.init(string: datas[1])
        self.quantity = String(format: "%f", multiplierNumber.doubleValue)
    }
